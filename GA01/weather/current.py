...
# Weather data format defaults to 'us', but can be optionally set as 'si'
...
def _sendRequest(lat,lng, format):
    if format == 'us':
        ...
        format = 'metric'
    r = requests.get('http://api.openweathermap.org/data/2.5/weather?appid='+_openWeatherApiKey+'&lat='+lat+'&lon='+lng+'&units='+format)
    data=r.json()
    return data
