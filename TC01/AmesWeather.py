...
def p(raw, type, format, wdate, isFirst):
    ...
    if raw == 'P':
        if type == 'current':
            info = current.getPressure(format)
            response = info['data']
            dstring = info['time']
            ...
            if stamp > nowStamp:
                ...
                info = historical.getPressure(str(stamp), format)
                response = info['data']
                dstring = info['time']
    else:
        offsetType = raw[-1:]
    ...
    if isFirst == 1:
        if dstring == 'N/A':
            ...
            dateObj1 = stampToDate(dstring)
            dateString = dateToString(dateObj1)
    ...
    if raw == 'T':
        if type == 'current':
            ...
            if stamp > nowStamp:
                ...
                info = historical.getTemp(str(stamp), format)
                response = info['data']
                dstring = info['time']
    else:
        offsetType = raw[-1:]
...
def ws(raw, type, format, wdate, isFirst):
    ...
    if raw == 'WS':
        if type == 'current':
            info = current.getWindSpeed(format);
            response = info['data']
            dstring = info['time']
        elif type == 'historical':
            now = datetime.datetime.now()
            ...
            if stamp > nowStamp:
                ...
                info = historical.getWindSpeed(str(stamp), format)
                response = info['data']
                dstring = info['time']
    else:
        offsetType = raw[-1:]
...
def wd(raw, type, format, wdate, isFirst):
    ...
    if raw == 'WD':
        if type == 'current':
            info = current.getWindDir(format)
            response = info['data']
            dstring = info['time']
        elif type == 'historical':
            now = datetime.datetime.now()
            ...
            if stamp > nowStamp:
                ...
                info = historical.getWindDir(str(stamp), format)
                response = info['data']
                dstring = info['time']
    else:
        offsetType = raw[-1:]
